...
high-frequency energy ratio---under an \emph{availability-aware} fusion rule:
a signal that cannot run is removed from the weighted average and the
remaining weights are renormalised, rather than being scored as zero. On a
...
reported as counts and as a functional check on the provenance branches---never
...
\begin{tabular}{@{}lccc@{}}
\toprule
Model & Correct & Mean, gen. & Mean, real \\
\midrule
\texttt{haywoodsloan/ai-image-detector-deploy} & 12/12 & 0.987 & 0.002 \\
\texttt{Ateeqq/ai-vs-human-image-detector}     & 11/12 & 0.752 & 0.001 \\
\texttt{dima806/ai\_vs\_human\_generated\_...} &  8/12 & 0.012 & 0.011 \\
\texttt{Organika/sdxl-detector}                &  5/12 & 0.438 & 0.574 \\
\texttt{dima806/ai\_vs\_real\_image\_detection}&  4/12 & 0.766 & 0.694 \\
\bottomrule
\end{tabular}
\end{table}
...
Track~A. The two generated curves coincide exactly---the centre crop does not
touch a natively $512\times512$ image---while the real distribution moves
toward zero under normalisation. The fraction of real images scoring above 0.5
...
one in two hundred---which is a different product, not a tuned one. The single
...
were---a split with a clear mechanism, namely an SDXL-specific detector
recognising SDXL-family output and passing SD~1.5-era fine-tunes and textual
inversions at near-zero confidence. The replacement misses thinly and
...
characters. In a few hundred kilobytes of compressed image data, a
...
found'' at score 1.0---full confidence---on \textbf{62 of 200 real
...
0.8461 normalised while frequency alone---carrying the smallest weight,
